...
def cargar_caso_demostrativo(parametros, especies, costos) -> None:
    vida, costos_especies = [], []
    for referencia in especies[:3]:
        registro, costo = filas_especie_desde_referencia(referencia, 2)
        vida.append(registro)
        costos_especies.append(costo)
    servicios_demo = [fila_servicio_desde_parametro(p) for p in parametros[:3]]
    costos_demo = []
    for cuenta in ("C", "D", "E", "R"):
        referencia = next((c for c in costos if texto(c.get("CUENTA")) == cuenta), None)
        if referencia:
            costos_demo.append(fila_costo_desde_referencia(referencia))
    st.session_state.vida_df = pd.DataFrame(vida)
    st.session_state.servicios_df = pd.DataFrame(servicios_demo)
    st.session_state.costos_df = pd.DataFrame(costos_especies + costos_demo)
    st.session_state.nombre_caso = "DEMO-ESVD-001"
    st.session_state.ubicacion = "Sitio ficticio A"
    st.session_state.objetivo = (
        "Caso totalmente ficticio para probar selectores, autocompletado, escenarios y exportación."
    )
    st.session_state.permitir_agregacion_ui = True
...
with st.sidebar:
    ...
    modo_demostracion = any(
        texto(p.get("ID_PARAMETRO")).startswith("ESVD-DEMO-") for p in parametros
    )
    ...
    st.caption("Carga un caso ficticio con valores en todas las cuentas para comprobar el flujo completo.")
    if st.button(
        "Cargar caso completo de demostración",
        use_container_width=True,
        disabled=not (modo_demostracion and parametros and especies_referencia and costos_referencia),
    ):
        cargar_caso_demostrativo(parametros, especies_referencia, costos_referencia)
        st.rerun()
...
with tab_caso:
    ...
    objetivo = st.text_area(
        "Descripción breve del evento y del daño",
        placeholder="Qué ocurrió, qué receptores fueron afectados y durante qué periodo.",
        key="objetivo",
    )
...
with tab_vida:
    ...
    st.write(
        "Seleccione una referencia para autocompletar el inventario y una línea monetaria de Cuenta A. "
...
with tab_costos:
    ...
    if costos_referencia:
        cuentas_disponibles = [c for c in ["A", "C", "D", "E", "R"] if any(texto(x.get("CUENTA")) == c for x in costos_referencia)]
